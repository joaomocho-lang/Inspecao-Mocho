...
function exportarPDF(){const{jsPDF}=window.jspdf;const doc=new jsPDF();const cabecalho=JSON.parse(localStorage.getItem('dadosCabecalho'));doc.setFontSize(18);doc.text('Relatório de Inspeção',10,10);let y=20;doc.addImage('Captura de ecrã 2025-11-17, às 22.07.23.png','PNG',150,5,40,40);doc.setFontSize(12);doc.setTextColor(0,0,255);doc.text('Informações do Cliente',10,y);y+=10;doc.setTextColor(0,0,0);for(const[k,v]of Object.entries(cabecalho)){doc.text(`${k}: ${v}`,10,y);y+=8;if(y>270){doc.addPage();y=10;}}doc.addPage();doc.setTextColor(255,140,0);doc.text('Respostas do Formulário',10,10);doc.setTextColor(0,0,0);y=20;for(const[pergunta,dados]of Object.entries(respostas)){doc.text(`${pergunta}: ${dados.resposta||'Não preenchido'}`,10,y);y+=8;if(y>270){doc.addPage();y=10;}if(dados.fotoPreview){doc.addImage(dados.fotoPreview,'JPEG',10,y,50,50);y+=60;if(y>270){doc.addPage();y=10;}}}doc.addPage();doc.text('Assinatura do Inspetor:',10,20);doc.text(cabecalho.assinatura||'',10,30);doc.save('relatorio.pdf');}

async function exportarWord(){const{Document,Packer,Paragraph,TextRun}=docx;const doc=new Document();const elementos=[];const cabecalho=JSON.parse(localStorage.getItem('dadosCabecalho'));elementos.push(new Paragraph({children:[new TextRun({text:'Relatório de Inspeção',bold:true,size:28})]}));for(const[k,v]of Object.entries(cabecalho)){elementos.push(new Paragraph(`${k}: ${v}`));}for(const[pergunta,dados]of Object.entries(respostas)){elementos.push(new Paragraph({children:[new TextRun({text:`${pergunta}: `,bold:true}),new TextRun(dados.resposta||'Não preenchido')]}));if(dados.foto){elementos.push(new Paragraph(`Foto anexada: ${dados.foto}`));}}doc.addSection({children:elementos});const blob=await Packer.toBlob(doc);const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='relatorio.docx';a.click();}
